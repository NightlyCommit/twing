...
describe("Twig.js Blocks ->", function () {
    ...
    it("should make the contents of blocks available after they're rendered", function (done) {
        ...
        twig({
            ...
            load: function (template) {
                ...
                var tokens = template.render({place: "block"}, {output: 'blocks'}).msg.map(function (token) {
                    return '' + token.markup + '';
                });

                tokens.should.eql(['Coming soon to a ', 'block', ' near you!']);
                done();
            }
        });
    });
    ...
    it("should work inside a loop", function (done) {
        twig({
            path: 'test/templates/blocks-with-loop.twig',
            load: function (template) {
                template.render().should.equal('HEADER\n\n    PARENT\n\n    A1    A2');
    ...
    it("should work when overriding a block inside a loop", function (done) {
        twig({
            path: 'test/templates/extends.twig',
            load: function (template) {
                template.render().should.equal('HEADER\n\n    CHILD\n\n    B1    B2');
