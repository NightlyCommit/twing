...
let fixturesPath = nodePath.resolve('test/tests/loader/fixtures');

let basePaths = [
    [
        fixturesPath,
        'test/tests/loader/fixtures/named_quater/named_absolute.html',
        null
    ],
    [
    ...
    [
        ...
        nodePath.join(process.cwd(), 'test/tests/loader')
    ],
    [
        'fixtures',
        'fixtures/named_quater/named_absolute.html',
        nodePath.join(process.cwd(), 'test/../test/tests/loader')
    ]
];
...
tap.test('loader filesystem', function (test: Test) {
    test.test('getSourceContext', function (test: Test) {
        let path = nodePath.resolve('test/tests/fixtures');
    ...
    test.test('security', function (test: Test) {
        for (let securityTest of securityTests) {
            ...
            let loader = new TwingLoaderFilesystem([nodePath.resolve('test/tests/fixtures')]);
