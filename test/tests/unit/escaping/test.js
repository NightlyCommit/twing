const {twingFilterEscape} = require('../../../../build/core/filters/escape');
...
tap.test('escaping', function (test) {
    test.test('htmlEscapingConvertsSpecialChars', function (test) {
        for (let key in htmlSpecialChars) {
            ...
            test.same(twingFilterEscape(getEnvironment(), key, 'html'), value, 'Succeed at escaping: ' + key);
        }
    ...
    test.test('htmlAttributeEscapingConvertsSpecialChars', function (test) {
        for (let key in htmlAttrSpecialChars) {
            ...
            test.same(twingFilterEscape(getEnvironment(), key, 'html_attr'), value, 'Succeed at escaping: ' + key);
        }
    ...
    test.test('javascriptEscapingConvertsSpecialChars', function (test) {
        for (let key in jsSpecialChars) {
            ...
            test.same(twingFilterEscape(getEnvironment(), key, 'js'), value, 'Succeed at escaping: ' + key);
        }
    ...
    test.test('javascriptEscapingReturnsStringIfZeroLength', function (test) {
        test.same(twingFilterEscape(getEnvironment(), '', 'js'), '', 'Succeed at escaping: ""');

        test.end();
    });

    test.test('javascriptEscapingReturnsStringIfContainsOnlyDigits', function (test) {
        test.same(twingFilterEscape(getEnvironment(), '123', 'js'), '123', 'Succeed at escaping: "123"');
    ...
    test.test('cssEscapingConvertsSpecialChars', function (test) {
        for (let key in cssSpecialChars) {
            ...
            test.same(twingFilterEscape(getEnvironment(), key, 'css'), value, 'Succeed at escaping: ' + key);
        }
    ...
    test.test('cssEscapingReturnsStringIfZeroLength', function (test) {
        test.same(twingFilterEscape(getEnvironment(), '', 'css'), '', 'Succeed at escaping: ""');

        test.end();
    });

    test.test('cssEscapingReturnsStringIfContainsOnlyDigits', function (test) {
        test.same(twingFilterEscape(getEnvironment(), '123', 'css'), '123', 'Succeed at escaping: "123"');
    ...
    test.test('urlEscapingConvertsSpecialChars', function (test) {
        for (let key in urlSpecialChars) {
            ...
            test.same(twingFilterEscape(getEnvironment(), key, 'url'), value, 'Succeed at escaping: ' + key);
        }
    ...
    test.test('javascriptEscapingEscapesOwaspRecommendedRanges', function (test) {
        ...
        for (let chr = 0; chr < 0xFF; ++chr) {
            if (chr >= 0x30 && chr <= 0x39
                ...
                test.same(twingFilterEscape(getEnvironment(), literal, 'js'), literal);
            }
            else {
                let literal = codepointToUtf8(chr);
                ...
                if (immune.includes(literal)) {
                    ...
                    test.notSame(twingFilterEscape(getEnvironment(), literal, 'js'), literal);
                }
            }
        }
    ...
    test.test('htmlAttributeEscapingEscapesOwaspRecommendedRanges', function (test) {
        ...
        for (let chr = 0; chr < 0xFF; ++chr) {
            if (chr >= 0x30 && chr <= 0x39
                ...
                test.same(twingFilterEscape(getEnvironment(), literal, 'html_attr'), literal);
            }
            else {
                let literal = codepointToUtf8(chr);
                ...
                if (immune.includes(literal)) {
                    ...
                    test.notSame(twingFilterEscape(getEnvironment(), literal, 'html_attr'), literal);
                }
            }
        }
    ...
    test.test('cssEscapingEscapesOwaspRecommendedRanges', function (test) {
        for (let chr = 0; chr < 0xFF; ++chr) {
            if (chr >= 0x30 && chr <= 0x39
                ...
                test.same(twingFilterEscape(getEnvironment(), literal, 'css'), literal);
            }
            else {
                let literal = codepointToUtf8(chr);

                test.notSame(twingFilterEscape(getEnvironment(), literal, 'css'), literal);
            }
        }
